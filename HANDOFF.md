...
- **Camper Schedules** *(admin-only nav tab "Schedules", `#tab-sched`)* — search a camper → per-week M/T/W/R/F day toggles, auto-saved to `schedules.json` (overrides keyed by `name||bunk` → `{week: "MWF"}`). `process_report(schedule_overrides=...)` applies them to the selected week (replaces `days_sched`/`days`/`enrolled`) for week-aware reports. `/api/schedules` GET/POST are `@admin_required`; the tab is hidden for non-admins (rolling out to all users after testing). Verified end-to-end.
