...
- **Camper Schedules** *(admin-only card `#sched-card`, sits in Utilities just below the Master Sheet section)* — search a camper → per-week M/T/W/R/F day toggles. A **scope** radio applies a day change to "this week only" or "this week + all later weeks" (`schedTargetWeeks`). Edits are local until the **💾 Save schedule** button persists them (also auto-saves on Back / switching campers so nothing is lost). Stored in `schedules.json` (overrides keyed by `name||bunk` → `{week: "MWF"}`; `""` = attends no days that week). `process_report(schedule_overrides=...)` applies them to the selected week (replaces `days_sched`/`days`/`enrolled`) for week-aware reports. `/api/schedules` GET/POST are `@admin_required`; POST accepts `week`/`weeks`+`days`, `clear`, or `replace` (full week map for Save). Card hidden for non-admins (rolling out to all users after testing — flip the `showApp` `is_admin` gate). Verified end-to-end.
