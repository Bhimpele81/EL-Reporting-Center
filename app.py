...
_DEFAULT_PRICING = {
    "season_label": "2026",      # current season label
    "proposed_label": "2027",    # proposed/upcoming season label
    "camp": {
...
def api_pizza_bunk_counts():
    ...
    for camp in config.get("camps", []):
        ...
        for b in camp.get("bunks", []):
            ...
            elif cl.startswith("upper") or cl.startswith("ft cit"):
                out["groups"]["upper"].append(entry)
            # other unmapped bunks are omitted
    return jsonify(out)
...
// Optional tuition add-ons (multi-select), e.g. CIT discount. Extensible.
const ADDON_OPTS = [['cit','CIT']];
const aoSummary = addons => { const n = (addons||[]).length; return n ? 'Add-ons ('+n+')' : 'None'; };
// Extended-care time slots (value = time key in pricing.extended, label shown)
const AM_SLOTS = [['','None'],['7:00','after 7:00'],['7:30','after 7:30'],['8:00','after 8:00'],['8:30','after 8:30']];
const PM_SLOTS = [['','None'],['4:30','before 4:30'],['5:00','before 5:00'],['5:30','before 5:30']];
...
function renderPxCalc() {
  ...
  h += '<div class="px-controls">' +
    '<label class="px-field">Season<select id="px-cal-season">' +
      `<option value="proposed"${pxSeason==='proposed'?' selected':''}>Proposed (${famEsc(pricing.proposed_label||'')})</option>` +
      `<option value="current"${pxSeason==='current'?' selected':''}>Current (${famEsc(pricing.season_label||'')})</option></select></label>` +
    '<label class="px-field">Rate<select id="px-tier">' +
    ...
    '</div>';
  const rateTypes = [['standard','Standard (2nd Grade+)'],['junior','Junior Camp (PS-1st)'],['cit','Full-Time CIT'],['pre','Year Round Childcare'],['sib','CC Sibling/Alumni'],['1yr','1st Year Childcare'],['1yrsib','1st Year CC Sibling']];
  ...
  document.getElementById('px-cal-season').addEventListener('change', e => { pxSeason = e.target.value; renderPxCalc(); });
...
function renderPxCalcTotal() {
  ...
  pxCampers.forEach((c,i) => {
    ...
    if (ptype === 'pre' || ptype === 'sib' || ptype === '1yr' || ptype === '1yrsib') {
      ...
      const label = firstYr ? (isSib ? '1st Year CC Sibling' : '1st Year Childcare') : (isSib ? 'CC Sibling/Alumni' : 'Year Round Childcare');
    ...
    // already-discounted Year Round Childcare / CC Sibling rate or the CIT discount.
  ...
  const seasonLbl = pxSeason==='current' ? (pricing.season_label||'Current') : (pricing.proposed_label||'Proposed');
  const html = '<div class="px-total-box">'+lines.join('')+'<div class="px-line px-sub"><span>Family total ('+famEsc(seasonLbl)+', '+tierLbl+')</span><span class="px-grand">'+pxMoney(campGrand)+'</span></div></div>';
...
function renderPxRates() {
  ...
  const curL = pricing.season_label || 'Current', propL = pricing.proposed_label || 'Proposed';
  const cur = pricing.camp.current || {ES:{}, Final:{}};
  ...
  h += '<div class="px-sec"><div class="px-sec-title">Childcare / School (weekly)</div>' +
    '<div style="font-size:.8rem;color:#888;margin-bottom:.3rem">Weekly summer rate a year-round childcare student pays, and the weekly rate their CC sibling/alumni pays. These are separate per-child rates, not a combined total. These rates are already discounted, so the 10% camp sibling discount is never applied on top of them.</div>' +
  ...
  h += '<div class="px-sec"><div class="px-sec-title">1st Year Childcare / School (weekly)</div>' +
    ...
    '<table class="px-tbl"><thead><tr><th class="px-l">Days/wk</th><th>1st Year Childcare</th><th>1st Year CC Sibling</th></tr></thead><tbody>';
  ...
  const exTable = (title, side, slots) => {
    let t = '<div class="px-sec"><div class="px-sec-title">'+title+'</div><table class="px-tbl"><thead><tr><th class="px-l">Time</th><th>5-day</th><th>4-day</th><th>3-day</th></tr></thead><tbody>';
    slots.forEach(([v,l]) => { if (!v) return; const r = (ex[side]||{})[v] || {};
      t += '<tr><td class="px-l">'+l+'</td><td>'+inp('px-ex-'+side+'-'+v+'-5', r['5'])+'</td><td>'+inp('px-ex-'+side+'-'+v+'-4', r['4'])+'</td><td>'+inp('px-ex-'+side+'-'+v+'-3', r['3'])+'</td></tr>'; });
...
function pxUpdateRateChange() {
  pricing.camp.week_order.forEach(w => {
    [['ES','ES'], ['Final','Final']].forEach(([liveKey, curKey]) => {
      const c = pxRateNum('px-cur-'+curKey+'-'+w), p = pxRateNum('px-camp-'+liveKey+'-'+w);
      const cell = document.getElementById('px-chg-'+liveKey+'-'+w);
      if (!cell) return;
...
async function savePxRates() {
  ...
  const curLbl = document.getElementById('px-season-cur'), propLbl = document.getElementById('px-season-prop');
  ...
  pricing.extended = pricing.extended || {am:{}, pm:{}};
  [['am',AM_SLOTS],['pm',PM_SLOTS]].forEach(([side,slots]) => {
    pricing.extended[side] = pricing.extended[side] || {};
  ...
  try {
    ...
    PZ_PERIODS.forEach((_,p) => {
      ...
      PZ_GROUPS.forEach(([g,,rate]) => {
        ...
        const b=[]; document.querySelectorAll('.pzb-bunks input[type=checkbox][data-p="'+p+'"][data-g="'+g+'"]').forEach(cb => { if (cb.checked) b.push(cb.value); }); o.bunks=b;
        st[p][g] = o;
      });
...
function pzbBuild() {
  ...
  PZ_PERIODS.forEach((title,p) => {
    ...
    PZ_GROUPS.forEach(([g,label,rate]) => {
      const gs = ps[g] || {};
      const isSpec = (g === 'specialists');
      const note = isSpec ? 'select areas' : (rate != null ? (rate+' slices/camper') : 'staff only');
